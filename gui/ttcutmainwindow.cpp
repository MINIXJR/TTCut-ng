...
: QMainWindow()
{
  ...
  connectStreamPointSignals();
  connectVideoSliderSignals();
  connectFrameAndCutListSignals();
  ...
  connectAVDataSignals();
}

// Static pixmaps of the original TTCut plus theme icons with a Qt standard-icon fallback.
void TTCutMainWindow::setupImagesAndIcons()
{
  // images
  // --------------------------------------------------------------------------
...
void TTCutMainWindow::restoreWindowGeometry()
{
...
// Main-menu actions and the recent-projects submenu.
void TTCutMainWindow::connectMenuSignals()
{
...
// Navigation widget (frame stepping, searches, logo ROI, quick jump) to the frame widgets and this window.
void TTCutMainWindow::connectNavigationSignals()
{
...
// Stream-point widget requests.
void TTCutMainWindow::connectStreamPointSignals()
{
...
// Video slider with the debounced decode timer.
void TTCutMainWindow::connectVideoSliderSignals()
{
...
// Cut-out frame, current frame and cut-list widgets.
void TTCutMainWindow::connectFrameAndCutListSignals()
{
...
// TTAVData notifications, including the dirty tracking that sets mProjectModified.
void TTCutMainWindow::connectAVDataSignals()
{
...
QString TTCutMainWindow::formatDurationMs(qint64 ms)
{
  if (ms < 0) ms = 0;
