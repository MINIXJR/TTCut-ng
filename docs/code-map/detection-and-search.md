...
|---|---|---|
| `PREV → MW → Task` (`preBuiltFrameIndex`) | `TTFrameIndexBundle` des Vorschau-Wrappers: die `QList<TTFrameInfo>` **plus** `isPAFF`/`frameMbsOnlyFlag`/`log2MaxFrameNum` (Qt-COW, O(1)). Leer ⇒ jeder Sub-Dekoder laesst sich den Index von einem eigenen `TTFrameIndexer` bauen und installiert ihn per `setFrameIndex()`. | Bei H.26x ist die Liste die **einzige** Quelle für Anzeige↔Dekodier-Zuordnung. Ein Worker ohne Index gibt aus `decodeFrame()` leere `QImage` zurück — genau der zweite der beiden Ur-Defekte der Pillarbox-Erkennung. |
