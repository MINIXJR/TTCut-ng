...
- Every reader of the audio header list takes **header 0** for the whole file
  ...
  (`frame_time` is samples / rate, so the grid holds for every constant
  sample rate); the acmod window turns time into a header index, which lags
  behind a skipped header.
