...
| [stream-points.md](stream-points.md) | Landezonen (Stream-Points) im Hauptfenster, die Verbraucherseite: wie Marker in `TTStreamPointModel` kommen (Analyse-Dispatch mit bis zu vier Tasks und Zähler `mStreamPointWorkersRunning`, Ergebnis-Slots, Handmarker, Projekt-Restore mit DISABLED-Annotation, VDR-/Defekt-Import), was das Widget damit tut (Sprung mit `onGotoFrame(idx, 0)` statt Slider, Cut-In/Out, Löschen, Audio-Reparatur), Speichern (alle Zeilen + `TTLogoProjectData`) und das Logo-Profil (`TTLogoDetector`: manuelle ROI aus 10 I-Frames mit 70-%-Kantenregel, markad-PGM bündig in die Ecke ohne Prüfung, Projekt-Rundreise baut die manuelle ROI neu). Verträge: Marker-Index = Anzeige-Domäne; Modell sortiert, dedupliziert nicht; `clearAutoDetected` verschont Hand-/VDR-/Error-Marker; Auto-Anomalie-Scan mit drei Einstiegen und Latch. Befunde (gelesen): zwei Worker ohne `deleteLater`; `"Marker (manuell)"` ohne `tr()`; Zeitspalte ohne Feldbild-Korrektur; drei markad-Ladepfade mit zwei Dekodierstrategien. | `76bbae80` | fresh (2026-09-13, erstellt) |
...
- Three markad logo loaders (`onLoadLogoFile`, `onLogoDataLoaded`, the auto-load in
  `onAVItemChanged`) repeat decodeFn/nextIFn, overlay and status handling with two
  decode strategies; two analysis-wrapper blocks (`onLogoDataLoaded`,
  `onLogoROISelected`); `onVideoPointsDetected`/`onAudioPointsDetected` are the same
  one-liner; four analysis-task start blocks share the connect/start/count sequence
  and two of them lack the `deleteLater` the others have.
