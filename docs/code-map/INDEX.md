...
From `output-mux.md`:
- The muxer is configured by hand at four sites (final H.26x cut, MPEG-2 MKV
  task, preview, playback) with the same five setters, and the frame duration
  is formatted as an `"<n>ns"` string at four sites — both consolidate
  candidates for audit run 7, together with the three variants of the ES
  deletion after a successful mux.
