...
- `onFileSave` and `onFileSaveAs` build the default project name and the save dialog
  twice (`askProjectFileName()` would also stop Save as from dropping the current
  target on cancel); `insertRecentFile` walks the top-level widgets although
  `TTSettings::recentFilesChanged` exists for the refresh and has no receiver.
