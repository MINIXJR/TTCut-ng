...
From `audio-es-input.md`:
- Every reader of the audio header list takes **header 0** for the whole file
  (cut grid, repair seconds, acmod window, list columns, compatibility check)
  and converts between time and header index with one fixed frame time. Holds
  for 48 kHz without holes in the list; not at 44.1 kHz (H1), for E-AC3 (H2) or
  after a skipped or missing header (H3).
