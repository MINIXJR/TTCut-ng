...
- Four sites derive the Working-Set from the per-codec App-Defaults (`TTSettings::load`, `TTSettings::setEncoderCodec`, `TTCutAVCutDlg::onResetDefaults`, `TTCutSettingsEncoder::loadCodecSettings`) and three map a stream type to the codec index (`onAudioVideoCut`, `onAVItemChanged`, `runAutoCutMode`) — candidates for one `syncWorkingSetToCodec` and one stream-type helper.

From `stream-open-project-load.md`:
- Five sites assign or emit the current AV item (`onOpenVideoFinished`, `onOpenAVStreamsAborted`, `onChangeCurrentAVItem`, `onReadProjectFileFinished`, `onReadProjectFileAborted`) — candidate for one `setCurrentAVItem`; the `.info` file is loaded twice per open.
