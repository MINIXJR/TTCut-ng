...
| [ttcut-demux.md](ttcut-demux.md) | TS→ES-Demux-Pipeline (Bash) mit Mess- und Meldekette: Timestamp-Repair, parallele Extraktion (+MPEG-2 Leading-B-Skip, Null-Truncation), `ttcut-pts-analyze` (3 Methoden; Grid-Methode kann TS-Korruption nicht von Field-Pictures unterscheiden), Gap-Erkennung + Silence-Insert, A/V-Dauerabgleich + End-Padding, `.info`-Erzeugung und wer welche Felder konsumiert (`TTESInfo`→`TTAVData`: `es_extra_frames` hat Vorrang vor Parser-Extras, speist Audio-Korrektur UND GUI-„Defekt:"-Marker; Dauer-Felder sind reine Menschen-Info). **Kartierte Defekte (TODO Medium):** `VIDEO_DURATION` = Container-Spanne statt Video-Dauer → Frame-Zahl rückgerechnet, Über-Padding, zirkuläre Drift-Meldung; „defective regions"-Fehldiagnose bei Feldpaaren. | `fe5e207` | fresh (2026-07-12: neu, aus dem Futurama-Melde-Audit; Mermaid LR 1.78 vs TD 2.12) |
