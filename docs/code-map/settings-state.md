---
base_commit: 094c32092b4b3db11378fcd0faa334e5d0f0c279
...
The legacy key `Muxer\OutputContainer` (`outputContainer()`) is still loaded
and saved but has no setter any more: it only feeds the `default:` branches of
the cut dialog's codec switch (an unknown codec index). The per-codec
`*Muxer` keys replaced it.
...
| `QS` → `DEF` | `load()` reads every key with the CURRENT field value as fallback (`settings.value(key, mField)`). Invariant that follows: **a key absent on disk can never reset a field** — a second `load()` in the same process inherits whatever the field holds (why `test_container_sync` spawns one process per case). Legacy migrations happen here: MP4 container value 2 → 1 for the three `*Muxer` keys and `OutputContainer`; `Mpeg2Preset/Profile` and 28 orphan keys are removed; thresholds are rounded to three decimals; `cutDirPath` falls back to `QDir::currentPath()` when the directory is gone; `WorkerCount` is bounded to 0..16. Side effect: `TTMessageLogger::setLogFilePath(mLogFilePath)`. |
| `DEF` → `QS` | `save()` writes all 76 keys, never `projectFileName`/`cutVideoName`/the Working-Set. Call sites: `TTCutMainWindow::closeEvent`, `openSettingsDialog` (after `exec()`, **regardless of OK or Cancel**), `onAudioVideoCut` (only after the cut dialog was accepted — moved there on 2026-08-18 because `closeProject`'s `load()` used to revert unsaved dialog edits). |
| `DEF` → `WORK` | The codec → (preset, CRF, profile, container) sync. In `load()` it runs unconditionally from the persisted `encoderCodec`; in `setEncoderCodec(v)` only when `v != mEncoderCodec`; in `TTCutAVCutDlg::onResetDefaults` on the button. MPEG-2 syncs only CRF and container (no preset/profile knob). The other six working mux fields are copied from their defaults in `load()` and `onResetDefaults` only — `setEncoderCodec` leaves them alone. `workingOutputContainer` comes from the per-codec `*Muxer` key, never from the legacy `OutputContainer` (that was the 2026-08-16 defect: fresh config cut MPEG-2 to MKV). |
| `SDLG` → `DEF` | `TTSettingsDialog::accept` calls the eight pages' `saveTabData` in a fixed order (encoder before muxer, "codec-dependent container logic"), then `QDialog::accept`; the caller saves to disk afterwards. Pages write persistent fields only — never the Working-Set, so a running project's overrides survive a settings edit. Exception: `TTCutSettingsMuxer::onMkvChaptersChanged` writes `setMkvCreateChapters` on every checkbox toggle, outside `accept`. |
| `OPEN` → `WORK` | `onAVItemChanged` maps the stream type to the codec index (h264 → 1, h265 → 2, else 0) and calls `setEncoderCodec`. Skipped entirely when `avItem == mpCurrentAVDataItem` or the item has no video stream. Because of the early return in `setEncoderCodec`, a stream of the SAME codec as the current one re-syncs nothing. |
...
| `PIPE` → `ID` | `onH26xCutFinished` and `onMpeg2MuxFinished` call `setCutVideoName(<final output file name>)` for the completion notification. After a cut the field therefore holds a CONTAINER name (`foo_cut.mkv`), no longer the ES name. |
...
| `HEAD` → `ID`/`WORK` | `runAutoCutMode` opens the project (all of the above runs), waits, then overwrites `cutDirPath`/`cutVideoName` from the CLI and calls `setEncoderCodec(streamType)` — a no-op after the project load already set that codec, so the project's Working-Set survives; `onDoCut` is called with the CLI target directly. No cut dialog, no `save()`. |
...
- **codec → (preset, CRF, profile, container) sync**
  - sites: `common/ttsettings.cpp:TTSettings::load`, `common/ttsettings.cpp:TTSettings::setEncoderCodec`, `gui/ttcutavcutdlg.cpp:TTCutAVCutDlg::onResetDefaults`, `gui/ttcutsettingsencoder.cpp:TTCutSettingsEncoder::loadCodecSettings` (into the UI, not the fields)
  - shared purpose: copy the per-codec App-Defaults into the Working-Set for a given codec index
  - status: consolidate → one `TTSettings::syncWorkingSetToCodec(int)` called by `load()`, `setEncoderCodec()` and the reset button; `loadCodecSettings` reads the result instead of switching itself
- **stream type → encoder codec index**
  - sites: `gui/ttcutmainwindow.cpp:TTCutMainWindow::onAudioVideoCut`, `gui/ttcutmainwindow.cpp:TTCutMainWindow::onAVItemChanged`, `gui/ttcutmainwindow_headless.cpp:TTCutMainWindow::runAutoCutMode`
  - shared purpose: h264_video → 1, h265_video → 2, else 0, then `setEncoderCodec`
  - status: consolidate → one helper (`TTAVTypes` or `TTSettings::setEncoderCodecForStream`); the two later calls are no-ops after `onAVItemChanged` anyway
- **default cut name from the video file**
  ...
  - status: kept separate → the headless name is a CLI argument, not a derivation
- **per-codec container preference vs legacy global container**
  - sites: `common/ttsettings.cpp` fields `mMpeg2Muxer/mH264Muxer/mH265Muxer` vs `mOutputContainer` (+ `outputContainerChanged` signal, never emitted since the setter went)
  - shared purpose: which container a cut produces
  - status: consolidate → drop `OutputContainer` (keep reading it once for migration), remove the dead signal; the cut dialog's `default:` branches then fall back to MKV
- **Working-Set mirror of the mux defaults**
  - sites: `common/ttsettings.cpp:TTSettings::load` (six assignments), `gui/ttcutavcutdlg.cpp:TTCutAVCutDlg::onResetDefaults` (same six)
  - shared purpose: reset the seven working mux fields to their defaults
  - status: consolidate → part of the `syncWorkingSetToCodec` helper above (the six codec-independent fields plus the container)
