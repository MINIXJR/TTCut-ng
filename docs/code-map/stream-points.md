...
| Detectors → `onPointsDetected` | `QList<TTStreamPoint>` with `frameIndex` in the **navigation (display) domain** (`detection-and-search.md`: positions from `moveToNext/PrevIndexPos`; the anomaly scan converts seconds with `videoFrameForTime`, extras included, and so does `TTStreamPointAudioWorker` for silence and audio-change markers — with the extra-frame list `onAnalyzeStreamPoints` passes it and, for a change, the header's `abs_frame_time` (audio-es-input.md). The two workers emit only when not aborted; `TTAudioAnomalyScanTask` emits an **empty** list on cancel and on an undecodable/non-48-kHz track; `TTAspectScanTask` emits its partial list on abort. One slot for every detector and for the imports below. |
