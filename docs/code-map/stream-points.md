...
| Widget → `TTAudioRepairDialog` | Context menu on an `AudioAnomaly` marker only, and only while an AV item with an AC3 track is injected (`setAVItem`, `setExtraFrameIndices` from `onAVItemChanged`; cleared in `closeProject`). Offers "Repair…" or, when `TTAVItem::findAudioRepairOverlapping` finds an existing `TTAudioRepairItem` on that track within the marker's approximate AC3 range (the dialog uses the same lookup to prefill), "Edit repair…"/"Remove repair". Accepting appends the "(repair planned)" suffix unless a variant is already there; removing strips it. The repair itself lives in `TTAVItem::audioRepairList`, not in the marker. |
...
- **analysis task start**
  - sites: `gui/ttcutmainwindow.cpp:TTCutMainWindow::startAnalysisTask` (callers `onAnalyzeStreamPoints` ×3, `startAudioAnomalyScan`)
  - shared purpose: connect `finished`/`aborted` to the counter slot and to `deleteLater`, start on the pool, count the worker
  - status: done `f2198216` (audit run 4, batch B); gate `test_analysis_task_lifetime`
