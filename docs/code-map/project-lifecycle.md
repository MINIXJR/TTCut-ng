...
| Headless → `openProjectFile` | `runAutoCutMode`: `setNonInteractive(true)`, open, `waitForProjectLoad(60 s)` (pumps until the flag falls), needs a non-empty cut list, sets the encoder codec from the stream type, then container, `cutDirPath`, `muxOutputPath` and `cutVideoName` from the CLI path (details in `settings-state.md`, edge `HEAD` → `ID`/`WORK`), connects `cutFinished → quit`, `onDoCut`. `runScreenshotMode`: open, `waitForProjectLoad(30 s)`, a warning on timeout but it carries on. Neither calls `save()` itself (the `quit()` of `--auto-cut` reaches `closeEvent`, which does); neither touches the recent list beyond what `onOpenProjectFileFinished` does. |
