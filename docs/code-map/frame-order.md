...
- **`TTCurrentFrame::createTempMkvForPlayback()`** (H.26x playback temp MKV) — requires `frameRate > 0`: after the `.info` fallback (`videoStream->frameRate()`, else `TTESInfo::frameRate()`) it bails, returning an empty path so the caller logs "playback unavailable" and aborts the play, rather than compute `frameDurationNs = 1e9 / frameRate` with `frameRate == 0` (UB) — `25c966eb`. Lifecycle: the temp file (`ttcut-ng_playback_temp.mkv`, fixed name) is reused across STOP→PLAY guarded by `playbackSourceFingerprint()`, and removed via `cleanupTempPlaybackFile()` both on cache-invalidation (source changed) **and** on `TTCurrentFrame` destruction (destructor added `25c966eb`) — so it is not left behind when the window is closed mid-playback.
