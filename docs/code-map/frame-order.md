...
flowchart TD
    ...
    R["TTMkvMergeProvider::mux() → assignEsTimestamps()<br/>(extern/ttmkvmergeprovider.cpp)<br/>pts = displayOrder[i] × duration (DISPLAY time)<br/>dts = (i − reorderOffset) × duration"]
...
|---|---|---|
| `TTFrameIndexer::mergePAFFFieldsInIndex` → `TTFrameIndexBundle::rawToMerged` → `TTH26xVideoStream::mapRawAuToDisplayIndex` | **A third index domain, distinct from both decode and display order:** `TTFrameIndexer::build` scans **one packet per raw AU** (PAFF top and bottom field are separate packets); the merge then collapses each field pair, so the bundle's `index` is *merged*. `.info` fields written by ttcut-demux (`es_doubled_pts_aus`) are numbered in **raw** AUs and must be translated before they can be compared against anything TTCut shows. Encoding: entry ≥ 0 → merged index; entry < 0 → collapsed bottom field with merged index `~entry`; empty vector = identity. **Correction (2026-09-05):** `rawToMerged`, `gops` and the display map are computed once, by `TTFrameIndexer::build()`, and then travel with the bundle wherever it goes — `TTH26xVideoStream::mFrameIndexBundle` is the canonical copy `mapRawAuToDisplayIndex()` reads, but `TTFFmpegWrapper::frameIndexBundle()` hands back the SAME whole bundle it adopted via `setFrameIndex()` (2026-09-04: "the wrapper stores the adopted bundle whole and re-exports it unchanged", per `avstream/ttframeindex.h`), `rawToMerged`/`gops` included — nothing is stripped on the round trip. What changed is only that `TTFFmpegWrapper` itself no longer exposes `rawPacketCount()`/`rawToMergedIndex()`/`rawIsCollapsedField()` convenience methods (removed from `extern/ttffmpegwrapper.h`); a caller that needs raw-AU translation goes through `TTFrameIndexBundle`'s own methods of the same name — via the owning `TTH26xVideoStream`, which is the only caller that does. | **RAW AU order** on the input side (one entry per demuxed packet), **merged decode order** on the output side — neither is display order |
| `TTH26xVideoStream::frameIndexBundle` → `TTFFmpegWrapper::setFrameIndex(const TTFrameIndexBundle&)` | PAFF flag, `frame_mbs_only_flag` and `log2_max_frame_num` travel **inside** the adopted index as one `TTFrameIndexBundle`. The two SPS values come from `TTNaluParser::parseH264SpsBasics` (called by `TTFrameIndexer::parseH264SpsFromExtradata`), which reads the de-escaped SPS through the shared walk `ttParseH264SpsHeader`; an over-long POC cycle or an SPS that ends before the flag leaves `frame_mbs_only_flag` unknown (`haveFrameMbsOnlyFlag = false`); an SPS that ends before `log2_max_frame_num` makes the call fail. Adopters never run the index pass themselves, so without them the decode-order tagging counts PAFF field packets as whole frames: `decodeFrame()` then never reaches a field-pair target AU and drains the file to EOF (measured 2×53 s in `46d3dcb1`; **recurred** through the quick-jump path and was measured again at 72 675 ms vs 13 ms, spec 2026-08-28). The pairing is no longer a call-ordering rule the caller must remember — `setFrameIndex(bundle)` is the only entry point and stores the bundle whole (2026-09-04), so index, metadata and display map cannot be separated. | n/a — carries stream properties, not indices; but it decides whether the adopter's **decode-order** tagging is correct |
...
| `TTVideoIndexList::moveToNextIndexPos(pos, type)` → TTVideoStream | next list position ≥ pos+1 matching frame type | **DECODE order** for H.26x (list built frame-by-frame from `TTH26xVideoStream::mFrameIndexBundle.index`, no POC sort — see the `TTVideoIndexList (H.26x)` assumption below); **display order** for MPEG-2 (list is sorted by `display_order` via `sortDisplayOrder()`) |
...
| `TTMPEG2Window2::moveToVideoFrame(iFramePos)` → `TTFFmpegWrapper::decodeFrame(iFramePos)` | integer `iFramePos` interpreted as index into `mBundle.index` (Owner B; the adopted `TTFrameIndexBundle`, since the 2026-09 wrapper split — the public `frameIndex()` accessor is unchanged) | **DECODE order** — the index was built by `TTFrameIndexer` scanning packets sequentially (decode order) |
...
| `TTFFmpegWrapper::decodeNearestKeyframe(displayPos, &shownDisplayPos)` → caller | `QImage` of the keyframe **at or before** `displayPos` (backward walk over `mBundle.index[].isKeyframe` in decode order from the mapped AU), plus `shownDisplayPos` = that keyframe's own display position (`decodeToDisplay(keyAU)`, or `keyAU` verbatim when the map is invalid) — **not** `displayPos` itself. `showKeyframeFastAt` propagates `shownDisplayPos` as its own return value, and callers (`onGotoFramePreview`) resync `videoStream`/`currentCutPosition` to that value, not the originally requested one. Decoded with `mSearchMode` forced true for the duration of the call (borrows the search path's no-DPB-prefill seek — safe because a keyframe is self-decodable) and saved/restored around it. Shares the `mFrameCache`/`mFrameCacheLRU` LRU cache with `decodeFrame()`, keyed by display position, so a keyframe already shown by either path is free on repeat | **DISPLAY order** out (`shownDisplayPos`); the returned frame is the nearest keyframe, not necessarily the requested position |
| `TTFFmpegWrapper::decodeFrame(n)` → caller | `QImage` shown in the still-image widget | **`n` is a DISPLAY position** (current state; the pre-fix decode-order behaviour is history, see "Root cause & knot" below). `decodeFrameInternal` maps `n` to `targetAU = displayToTargetAU(n)` (via the display-order map when valid, else identity) and decodes via `seekAndSkipToAU` until the delivered frame's `pts` tag equals `targetAU` — not by counting N decoder outputs. The shown content is therefore the true display frame n, matching what the Smart Cut engine would cut at display position n (WYSIWYG). |
| `TTFFmpegWrapper::decodeFrame(n)` → `mBundle.index[n].deliveredDecodeIndex` | the DECODE-order AU actually delivered for display position n | set as `index[n].deliveredDecodeIndex = targetAU` on a successful decode — exact by construction, since `targetAU` is also the loop's stop condition |
...
| `TTESSmartCut::outputDisplayOrder()` → caller → `TTMkvVideoOptions::displayOrder` → `TTMkvMergeProvider::setVideoOptions()` | `QVector<int>` — source display indices re-numbered to a compact, gap-free, order-preserving RANK (sort + `QHash` rank lookup) | **DISPLAY order**, output-local (segments leave gaps in source display numbering, so the absolute source index is not usable as a packet-count-based array index in the muxer) — returns an **empty** `QVector` (fallback) on: a negative `decodeToDisplay()` result (HEVC dropped-RASL slot, guarded in `appendOutputDisplay`), an encoder-packet/submitted-frame-count mismatch (`bufferAndWriteEncoderPacket`), or a duplicate source display index (`outputDisplayOrder`) — all three warn loudly via `TTMessageLogger` |
| `TTMkvMergeProvider::assignEsTimestamps(in)` | `pkt->pts = displayOrder[frameCount] × frame duration`; `pkt->dts = (frameCount − reorderOffset) × frame duration` — each computed in ns (`frameDurNs`) and rounded on its own to the 1 ms output time base (rounding the duration once drifted 1.1 % at 29.97 fps, gate `mkv_framerate`) | **PTS: DISPLAY order (true display time); DTS: approximated decode order** — `reorderOffset = max(i − displayOrder[i])` over the whole list (computed once in `setupVideoInput`) so `dts ≤ pts` always holds by lowering DTS, never raising PTS (raising PTS would shift video against the untouched audio streams); a resulting negative start DTS is normalized by `avoid_negative_ts` across **all** muxed streams together, not just video. Called once per output **frame** (`in.frameCount`) — PAFF field pairs are merged into one call by `processPAFFFieldPair()` first, matching `TTNaluParser`'s own field-pair merge 1:1 in count. Empty `displayOrder` (MPEG-2 task path: `data/ttavdata.cpp` MPEG-2 branch never calls `setVideoDisplayOrder`; playback temp MKV: `TTCurrentFrame::buildPlaybackMuxParams` fills `displayOrder` (applied by `TTPlaybackMuxTask::operation` via `setVideoDisplayOrder`) **only** for an H.26x stream with a valid display map — MPEG-2 playback or an invalid/empty H.26x map leave it empty, see the display-PTS bullet under *Redundancy*; or any invalidated H.26x run) falls back line-for-line to the pre-existing `pts = dts = frameCount × frameDur` linear stamping |
...
- **`TTFFmpegWrapper::decodeFrame(n)`** — assumes: `n` is a DISPLAY position (mapped via `displayToTargetAU()` to a decode-order AU in `mBundle.index`, the adopted `TTFrameIndexBundle`, when the display-order map is valid; identity otherwise). Guarantees: returns the QImage of the true display frame n — the fix described under "Root cause & knot" below, current since v0.72.0/2026-06-08. The caller (`TTMPEG2Window2`) and the cut-list agree on this domain: both pass display positions.
...
- **`TTFFmpegWrapper::seekAndSkipToAU(targetAU, displayIndex, caller)`** — the skip-loop body shared by `decodeFrame`'s and `decodeFrameYUV`'s non-sequential paths: `seekToFrame(targetAU)`, then a bounded `skipCurrentFrame()` loop until the delivered `pts` tag equals `targetAU`. Bound: `qBound(1, (targetAU − seekStart) + max(headroomEnd − targetAU, 256), index.size())`, where `headroomEnd` is the second keyframe past `targetAU` (or EOF) — reorder/field-pairing moves the target by at most a GOP, never by the length of the file. Polls `isCancelled()` (the wrapper's `mCancelToken`, set via `setCancelToken()`) once per output, returning `SeekSkip::Cancelled` immediately rather than continuing the loop. **Pitfall this replaces (fixed `8582c27d`, 2026-09-11):** before this fix, `decodeFrameYUV` ran its own separate inline skip loop bounded only by `mBundle.index.size()` (the whole file) with no cancel poll at all — the same "full decode to EOF" failure class the `skipCurrentFrame()` EAGAIN fix above addresses, but reachable specifically through the frame-search path (`TTFrameSearchTask`, `TTSearchTask`, both outside this map's `sources:`), and on that path uncancellable mid-decode until the search tasks' abort flags were wired in as cancel tokens (`f61baed4`, same date). `decodeFrame` itself already had the tight bound at `base_commit`, just not yet factored into a function shared with `decodeFrameYUV`.

- **`TTFFmpegWrapper::decodeNearestKeyframe(displayPos, &shownDisplayPos)`** — assumes: `displayPos` is a display position (mapped to AU via the display-order map when valid, else treated as a raw decode index, matching `decodeFrame`'s own fallback). Guarantees: returns the keyframe **at or before** the mapped AU, walking backward over `mBundle.index[].isKeyframe` in decode order — never the exact requested frame. `shownDisplayPos` is the display position of that keyframe, and it is the value callers must resync their own state to (`showKeyframeFastAt` returns it verbatim; `onGotoFramePreview` uses it, not the original `pos`, for `videoStream->moveToIndexPos()` / `currentCutPosition`). Pitfall: treating the return as "the frame at `displayPos`" is wrong by construction — it is a preview shortcut for slider-drag, paired with an exact `onGotoFrame()` call once the drag ends.
...
- **`TTCurrentFrame::buildPlaybackMuxParams()`** (H.26x playback temp MKV) — requires `frameRate > 0`: after the `.info` fallback (`videoStream->frameRate()`, else `TTESInfo::frameRate()`) it bails, returning false so `startPlaybackMux()` logs "playback unavailable" and aborts the play, rather than compute `frameDurationNs = 1e9 / frameRate` with `frameRate == 0` (UB) — `25c966eb`. The mux itself runs in `TTPlaybackMuxTask` on `QThreadPool::globalInstance()` (2026-09-11); `onPlaybackMuxFinished()` adopts the file and starts playback, `onPlaybackMuxAborted()` resets the buttons. Lifecycle: one file per mux (`ttcut-ng_playback_<pid>_<n>.mkv`) — a mux being discarded (`detachPlaybackMux()` → `TTPlaybackMuxTask::discard()`, on stream switch or widget destruction) may still be writing or removing its output while the next one starts; the finished file is reused across STOP→PLAY guarded by `playbackSourceFingerprint()`, and removed via `cleanupTempPlaybackFile()` both on cache-invalidation (source changed) **and** on `TTCurrentFrame` destruction (destructor added `25c966eb`) — so it is not left behind when the window is closed mid-playback.
...
to the ES, in write order, at all three write sites (stream-copy bulk/per-AU in
`streamCopyFrames`, and encoder packets in
...
duration` (true display time) and `dts = (i − reorderOffset) × duration`
(both computed in ns and rounded per packet), where
...
POC is reachable from the encoder's POC domain (`pocDomainBridgeable()`,
...
per-segment SPS unification when it is not. Since `1893497`, the encoder POC
width feeding that decision is *measured*, not assumed: `probeEncoderPocParams()`
opens a throwaway libx264 context (`AV_CODEC_FLAG_GLOBAL_HEADER`, same
options `setupEncoder()` would use) before any segment is encoded and reads
`log2_max_poc_lsb`/`poc_type` from its SPS extradata; `processSegment` uses the
probed width only when the probe reports `poc_type==0`, otherwise it falls
back to the constant `kExpectedEncoderLog2PocLsb` (4) — which is in fact the
common case: measured (`b0e1335`) that libx264 with progressive `bf=0` picks
`poc_type=2` (no POC-LSB field at all; `poc_type=0` only appears with the
interlace flags this probe sets for MBAFF/PAFF sources), so the classic seam
still runs on the constant, now confirmed rather than assumed; the probe
reports that measured `poc_type!=0` fact as an `infoMsg`, not a warning.
`parseEncoderSpsFromPacket` (`6cb520b`) cross-checks the probe against each
segment's REAL encoder SPS (parsed from its first output packet's extradata)
and warns loudly (`log2_max_poc_lsb` and `poc_type` both compared) if the two
diverge. When unification runs for such a
...
> (`TTDisplayOrderMap`, `TTFrameIndexBundle::index`, `mAccessUnits`) — here the affected
