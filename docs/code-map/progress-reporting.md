...
closed two things the map used to record as open: the MPEG-2 preview's cancel
button now reaches its nested `TTCutVideoTask` (previously not forwarded at
all — see the cut-preview pitfall below), and a failed MKV mux via mplex now
sets `mLastCutError` like every other failure path (previously silent,
...
- **`TTThreadTask::run()`'s `catch(const TTException&)` used to discard both
  ...
  fixes what used to be silently lost on a genuine failure of a *synchronous*
  nested task.
...
- **`TTFileBuffer::directWrite()` used to make a full-disk / unwritable-target
...
- **`doH264Cut`/`doAudioOnlyCut`** — **no longer synchronous** (`3f56d6e3`,
  ...
  and non-terminal. **Corrected 2026-08-15**: this bullet used to record an
  "Exit emitted before mLastCutError is assigned" ordering defect here,
  citing a `TODO.md` entry that no longer exists — `27abed4c` fixed exactly
  this by routing `onH26xCutFinished`/`onAudioOnlyCutFinished` (and every
  other closing site) through `TTAVData::finishCutOperation()`, which sets
  `mLastCutError` before emitting `Exit` (see that row above). The ordering
  is correct today; `onStatusReport`'s
...
- **`TTProgressBar`** — display only; no business logic, but now owns its
  ...
  the up-to-3 s the mplex step may take to stop its child process. The MPEG-2
  preview used to belong on this list too — a cancel was not forwarded to
  its nested `TTCutVideoTask` at all, only polled between clips (measured:
  a cancel armed 36 ms into a clip still let it finish 2.4 s later) — but
  `0bd07c93` closed that gap: `TTCutPreviewTask::onUserAbort()` now calls
  `cutVideoTask->onUserAbort()` directly, so the cancel lands inside the
  clip the moment its own poll point (`TTCutVideoTask::operation`'s
  `isAborted()`/`TTVideoStream` abort checks) is next reached — fast, not
  instant, so the guard may still matter for that residual gap, but the
  "not forwarded at all" claim this bullet used to make no longer holds
  (`67341cd1` retired the stale TODO entry that repeated it). `AddProcessLine` had to
...
- **`TTMplexProvider`** — emits only `ShowProcessForm`/`AddProcessLine`/
  ...
  **A failed mplex used to look like a success** (`0bd07c93`,
  ...
  reason a failed cut task does: `onProcFinished()` used to run its
...
- **`TTESSmartCut` progress** — percent is now work-weighted (ms/frame,
  ...
  first segment only when no seed is available at all. **`av_packet_alloc`
  failure used to truncate a segment while still reporting success**
  (`851e2286`): both `decodeFramesIntoList()` and `runEncodePass()` answered
  an allocation failure with `break` — the former left its feed loop early
