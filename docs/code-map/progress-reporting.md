...
- **Audio progress lambda pair** (`onProgress` callback for
  `cutAudioTracks`, same `(i*100+percent)/audioCount` folding): the two pool
  tasks share `TTAbortableTask::audioTrackProgress()` since code-audit run 7;
  the MPEG-2 branch of `onDoCut` (`data/ttavdata.cpp`) keeps its own copy
  because it emits `statusReport` with `qApp->processEvents()` on the GUI
  thread instead of `reportStep`. Engine progress is forwarded by
  `TTAbortableTask::forwardProgressOf()` (direct connection, reason written
  there), including the MKA mux, which reports progress since run 7.
...
- **Progress-bar creation + cancel wiring**: **Done, code-audit run 3
  (`9e5511f0`)**: `TTCutMainWindow::ensureProgressBar()` now holds the
  lazy `new TTProgressBar(this)` plus both `cancel` connections
  (`TTAVData::onUserAbortRequest` and `onAbortStreamPoints`), called from
  both the `Init` and `Start` branches of `onStatusReport` (each still a
  no-op after the first call — `if (progressBar != 0) return;`).
- **Stream → task status-report wiring**: `TTThreadTask` gained
  `linkStatusSource<Source>()`/`unlinkStatusSource<Source>()` template
  helpers (`common/ttthreadtask.h`) that `connect`/`disconnect` a stream's
  `statusReport(int, const QString&, quint64)` into the task's own
  `onStatusReport` overload — previously five call sites
  (`TTOpenVideoTask`, `TTOpenAudioTask`, `TTOpenSubtitleTask`,
  `TTCutVideoTask`, `TTCutTask`) each wrote the same `connect`/`disconnect`
  pair by hand. Only `TTCutVideoTask`/`TTCutTask` are inside this map's
  scope; the open tasks are not (see `stream-points.md`-adjacent code, out
  of scope here). No behaviour change — same signal, same slot, same
