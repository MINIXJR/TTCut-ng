...
- **`TTAVData` operation brackets (`mCutOperationActive`)** — the MPEG-2
  ...
  **two** pool runs (video task, then `TTMuxTask`), so `onCutFinished`
  ...
  and raises the damaged-recording dialog. Both `doCutPreview`'s
