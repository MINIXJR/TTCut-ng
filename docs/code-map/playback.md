---
base_commit: d592d195e98af6a9a3e36381ed7e8c38da1ff50e
...
| `SW` → `MP2` | Start seconds from `currentFrameTime()`; for MPEG-2 with field pictures replaced by `(index − extrasBefore(index)) / fps` (binary search in `extraIndices()`, the bitstream parser's list, NOT the `.info` count). Audio: `audioStreamAt(0)` as `--audio-file`; mpv seeks in the raw ES itself. |
...
| `MKV` → `TMK` → `LOAD` | `playbackSecondsForCurrentStill`: needs `deliveredDecodeIndex` of the still (lazily filled by the still decoder; −1 before the first decode → warning and `index / fps`); with display PTS the decode index is mapped to its display slot, else used as is — the `mTempPlaybackHasDisplayPts` flag keys every conversion so no mixed-scale state exists. Audio is inside the MKV, so `load()` gets no audio file. |
...
| `WR` → `POS` → `SLIDER` | `positionChanged` at observer rate (~10 Hz) → `floor(seconds × fps)`, clamped to `[0, frameCount)` → `updateCurrentPosition(pos)`: label, then `newFramePosition` → `TTCutMainWindow::onNewFramePos` (slider with blocked signals, **and** `TTCutFrameNavigation::checkCutPosition`, which stores the value as the position a Set Cut-In/Out would take) and `TTAVData::onCurrentFramePositionChanged`. The comment in `onPlaybackPositionChanged` says "no cut-position write"; `checkCutPosition` does store it. |
...
- **`TTCurrentFrame::onPlaybackPositionChanged`** — assumes one displayed frame per `1/fps` of mpv time; for MPEG-2 with field pictures the live index therefore drifts by the number of extras before the position, while `onPlaybackFinished` corrects the same quantity. Read-derived: the slider and the cut position taken during playback are off by that count on such material.
...
- **`TTMpvLibBackend::start`** — options are frozen at `mpv_initialize`: `setKeepOpen` after the first `renderWidget()`/`load()` has no effect (the preview sets it before). `hwdec=no` by default (`MPV_HWDEC` overrides); `gpu-hwdec-interop=no` in that case to keep ffnvcodec's `libcuda` complaint off stderr.
...
- **field-picture extras before an index (binary search over `extraIndices()`)**
  - sites: `gui/ttcurrentframe.cpp:TTCurrentFrame::onPlayVideo`, `gui/ttcurrentframe.cpp:TTCurrentFrame::onPlaybackFinished` (inside the fixed-point loop)
  - shared purpose: count MPEG-2 field-picture index entries below a position
  - status: consolidate → one `extrasBefore(index)` on `TTMpeg2VideoStream` (or a helper next to `extraIndices()`), and use it in `onPlaybackPositionChanged` as well, which lacks the correction today
- **start-seconds derivation**
  - sites: `gui/ttcurrentframe.cpp:TTCurrentFrame::onPlayVideo` (MPEG-2 branch), `gui/ttcurrentframe.cpp:TTCurrentFrame::playbackSecondsForCurrentStill` (H.26x)
  - shared purpose: seconds of the still for `--start`
  - status: consolidate → `playbackSecondsForCurrentStill` handles both codecs (the header already calls it "the conversion authority"), `onPlayVideo` calls it once before the codec switch
- **end-of-playback detection**
  - sites: `gui/ttmpvwrapper.cpp:TTMpvWrapper::onPropertyChanged` (`eof-reached`), `gui/ttmpvwrapper.cpp:TTMpvWrapper::onBackendPlaybackFinished` (`END_FILE`), `gui/ttmpvwrapper.cpp:TTMpvWrapper::stop`
  - shared purpose: set `mAtEnd`/`mPlaying` and emit `playerFinished` once
  - status: consolidate → one private `finishPlayback()` that the three call; the mutual guard then lives in one place
