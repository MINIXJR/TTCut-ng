...
bool TTESSmartCut::setupDecoder()
{
    ...
    // Feed extradata (SPS/PPS) to decoder
    QByteArray extradata;
    if (mParser.codecType() == NALU_CODEC_H265 && mParser.vpsCount() > 0) {
        extradata.append(mParser.getVPS(0));
    }
    if (mParser.spsCount() > 0) {
        extradata.append(mParser.getSPS(0));
    }
    if (mParser.ppsCount() > 0) {
        extradata.append(mParser.getPPS(0));
    }
