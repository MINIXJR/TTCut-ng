...
- **TTFrameIndexer** (avstream/ttframeindexer.h): builds the H.264/H.265 frame index bundle (packet scan, PAFF merge, GOP table, display-order map) that TTH26xVideoStream owns and every TTFFmpegWrapper adopts
...
- Subtitle export as a per-run option (`--subs`/`--no-subs`, default off): DVB bitmap subtitles → `.mks` (Matroska, stream copy) plus ccextractor-OCR → cuttable `.srt` (ES-timeline based, auto-loaded by TTCut); embedded SubRip → `.srt`; sources the ORIGINAL TS (the repaired TS carries no subtitle streams)
...
- `processSegment()` picks one of two reachable branches: **SPS-Unification** when `codecType() == H.264 && (isPAFF() || !pocBridgeable || seamNeedsUnification)` (the third trigger: a non-IDR copy start with leading pictures, even when the POC seam is bridgeable), otherwise the **standard** path (all H.265, plus progressive H.264 with a bridgeable POC seam and no such seam).
...
- SPS patching (`ttPatchH264SpsReorderFrames`, `extern/tth264bitstream.cpp`) is stream-type-aware via `isPAFF` parameter: PAFF streams get increased `num_ref_frames`/`max_dec_frame_buffering` for MBAFF→PAFF DPB transitions; non-PAFF streams keep original values.
